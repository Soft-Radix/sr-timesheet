...
// Define CORS headers with explicit configuration
const corsHeaders = {
...
async function verifyFolderAccess(folderId: string) {
  try {
    console.log('Verifying folder access for ID:', folderId);
    ...
    if (response.data.mimeType !== 'application/vnd.google-apps.folder') {
      console.error('Invalid folder type:', response.data.mimeType);
...
async function getOrCreateEmployeeSpreadsheet(userEmail: string) {
  console.log('Starting getOrCreateEmployeeSpreadsheet for:', userEmail);
  ...
  try {
    ...
    console.log('Searching for existing spreadsheet:', fileName);
    
    const response = await drive.files.list({
      q: `name = '${fileName}' and '${PARENT_FOLDER_ID}' in parents and mimeType = 'application/vnd.google-apps.spreadsheet' and trashed = false`,
    ...
    if (response.data.files && response.data.files.length > 0) {
      ...
      for (let i = 0; i < MONTHS.length; i++) {
        ...
        await sheets.spreadsheets.values.update({
          ...
          requestBody: {
            values: [['Date', 'Hours', 'Project', 'Description']],
          },
        });
      }
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!date || !hours || !project || !description || !userEmail) {
      console.error('Missing required fields:', { date, hours, project, description, userEmail });
