...
async function findSpreadsheet(userEmail) {
...
async function getTimesheetEntries(spreadsheetId, date) {
...
async function getAllUsers() {
  ...
  do {
    ...
    nextPage = data.nextPage; // Update nextPage for the next iteration
  }while (nextPage)
  return allUsers.map((user)=>user.email);
}
async function sendSlackNotification(reports) {
  ...
  const blocks = [
    ...
    ...reports.map((report)=>({
        ...
        text: {
          type: 'mrkdwn',
          text: report.status === 'missing' ? `⚠️ *${report.email}* has not submitted timesheet for today` : `⚠️ *${report.email}* has logged only ${report.hoursLogged} hours today (minimum required: 8 hours)`
        }
      }))
  ];
  try {
